...
$(function() {
  ...
  $('#j-nav li').on('click', function() {
    ...
    // 更新选中状态
    $('#j-nav li').removeClass('am-active')
    $(this).addClass('am-active')

    // 更新输入框
    $('#j-input')
      .data('filter', filter)
      .attr('placeholder', filter === 'name' ? '例如: 不要说话 陈奕迅' : filter === 'id' ? '例如: 25906124' : '例如: http://music.163.com/#/song?id=25906124')
    ...
    // 更新验证规则
    $('#j-validator').validator('setField', {
      'input': filter === 'url' ? 'url' : 'required'
    })
  ...
  // 平台选择按钮点击事件
  $('#j-type input[type="radio"]').on('change', function() {
    // 移除所有按钮的动画
    $('.explosion').removeClass('exploding');
    // 给当前选中的按钮添加动画
    $(this).next().next('.explosion').addClass('exploding');
    
    // 300ms后移除动画类，以便下次点击时重新触发
    setTimeout(() => {
      $('.explosion').removeClass('exploding');
    }, 300);
  });
          ...
          $.ajax({
            ...
            success: function success(result) {
              if (result.code === 200 && result.data) {
                ...
                var setValue = function setValue(data) {
                  ...
                  var $srcBtn = $('#j-src-btn');
                  var $lrcBtn = $('#j-lrc-btn');
                  if ($srcBtn.length && $lrcBtn.length && 'download' in $srcBtn[0]) {
                    var name = data.title + '-' + data.author;
                    $srcBtn.attr('download', name + '.mp3');
                    $lrcBtn.attr('download', name + '.lrc');
            ...
            complete: function complete() {
              ...
              if (page === 1) {
                ...
                $('#j-submit').text('搜一下').prop('disabled', false);
              }
            }
          });
        };
